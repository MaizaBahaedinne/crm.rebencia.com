...
class Wp_client_model extends CI_Model {
    ...
    public function all($limit=200, $offset=0, $filters=[]) {
        ...
        $sql = "SELECT u.ID, u.display_name AS nom, u.user_email AS email,
                ...
                MAX(CASE WHEN m.meta_key='houzez_buyer' THEN m.meta_value END) AS houzez_buyer,
                MAX(CASE WHEN m.meta_key='houzez_owner' THEN m.meta_value END) AS houzez_owner,
                MAX(CASE WHEN m.meta_key='houzez_seller' THEN m.meta_value END) AS houzez_seller
                FROM {$users} u
                LEFT JOIN {$umeta} m ON m.user_id = u.ID
                WHERE 1=1";
        $params = [];
        if(!empty($filters['q'])) {
            $sql .= " AND (u.display_name LIKE ? OR u.user_email LIKE ?)";
            $params[] = '%'.$filters['q'].'%';
            $params[] = '%'.$filters['q'].'%';
        }
        if(!empty($filters['type'])) {
            $sql .= " AND EXISTS (SELECT 1 FROM {$umeta} mt WHERE mt.user_id=u.ID AND mt.meta_key='fave_author_type' AND mt.meta_value=?)";
            $params[] = $filters['type'];
        }
